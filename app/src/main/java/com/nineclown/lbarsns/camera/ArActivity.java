        ...
        if (camera.getTrackingState() == TrackingState.TRACKING) {
            ...
            if (anchorNodeCollection != null) {
                for (AnchorNode anchorNode1 : anchorNodeCollection) {
                    ...
                    Vector3 vector31 = anchorNode1.getChildren().get(0).getChildren().get(0).getWorldPosition();
                    float[] fvec3 = new float[4];
                    fvec3[0]=vector31.x;
                    fvec3[1]=vector31.y;
                    fvec3[2]=vector31.z;
                    fvec3[3]=1.f;
                    float[] matrix = new float[16];
                    arFragment.getArSceneView().getArFrame().getCamera().getProjectionMatrix(matrix,0,1.f,1.f);
                    float scale = matrix[3]*fvec3[0]+matrix[7]*fvec3[1]+matrix[11]*fvec3[2]+matrix[15]*fvec3[3];
                    //anchorNode1.getChildren().get(0).getChildren().get(0).setLocalScale(new Vector3(1,1,1));
                }
            }
            ...
            if (!istest) {
                ...
                Quaternion quaternion3 = new Quaternion(startTrackingQuaternion[1],startTrackingQuaternion[2],startTrackingQuaternion[3],startTrackingQuaternion[0]);
                quaternion2= Quaternion.multiply((new Quaternion(quatenionAndroidPose[0],quatenionAndroidPose[1],quatenionAndroidPose[2],quatenionAndroidPose[3])),quaternion3.inverted());
                Quaternion var1 = new Quaternion(Vector3.right(), quaternion2.x);
                Quaternion var2 = new Quaternion(Vector3.up(), quaternion2.y);
                Quaternion var3 = new Quaternion(Vector3.back(), quaternion2.z);
                ...
                if (getAngle == 0d) {
                    ...
                    rotationQuaternionY = Quaternion.axisAngle(new Vector3(0f, 1f, 0f), (float)(180d*getAngle)/(float)Math.PI);
                }
                istest = true;
            }
            ...
            if (!isstart) {
                ...
                for (int i = 0; i < contentDTOs.size(); i++) {
                    ...
                    if (contentDTOs.get(i).getLatitude() == null || contentDTOs.get(i).getLongitude() == null) return;
                    ...
                    future.thenAccept( view -> {
                        ...
                        viewPanel.setLocalScale(new Vector3(0.15f ,0.15f ,0.15f));

                    });
                    andy.setOnTapListener(((hitTestResult, motionEvent) -> {
                        viewPanel.setEnabled(!viewPanel.isEnabled());
                        Vector3 andyvec3 = andy.getWorldPosition();
                        Vector3 cameravec3 = arFragment.getArSceneView().getScene().getCamera().getWorldPosition();
                        double scale = Math.sqrt((double)(andyvec3.x - cameravec3.x)*(andyvec3.x - cameravec3.x)+(andyvec3.z - cameravec3.z)*(andyvec3.z - cameravec3.z));
                        if(scale > 1.d) viewPanel.setLocalScale(new Vector3((float)scale/10f,(float)scale/10f,(float)scale/10f));
                    }));
                }
                isstart = true;
            }
